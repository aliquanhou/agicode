...
## 2.3.0 (2026-07-16) — Probe Panel: 左中右三栏格局

### 新增
- 🔍 **探针监控面板**（左侧栏）：实时显示运行状态、工具调用数、错误数、最近工具耗时柱状图
- 📋 **一键复制探针数据**：点击按钮复制完整探针报告到剪贴板，可粘贴给 AI 分析
- 📡 **实时事件流**：左侧栏底部实时滚动显示最近工具调用和错误
- 🏗️ **左中右三栏布局**：探针 | 渲染输出 | 工具面板

### 修复
- 🐛 **tools.py**: `execute_tool()` 集成探针记录，每次工具调用自动打点
- 🐛 **core.py**: Agent 构造时自动初始化探针
- 🐛 **web_server.py**: 新增 `GET /api/probe` 端点返回实时统计和事件摘要
...
### 核心
- 全透明 AI 工程智能体，每一步动作实时广播
- 零死循环保障（while True，模型自主决定停止）
- 5 大 LLM Provider：Anthropic · OpenAI · DeepSeek · Gemini · Ollama

### 透明输出层
- **transcript 事件总线**: session/phase/step/thought/tool/text/loop/error
- **workflow 状态机**: 计划创建→步骤分解→逐步执行→完成/失败
- GUI 工作流进度条：当前步骤、下一步、完成进度实时展示
- Claude Code 风格 UI 渲染：工具耗时、步骤摘要、下一步建议、代码块着色
- Unified Diff 渲染（编辑/写入自动生成 diff）+ 绿/红/紫着色
- 滚动指示器 "↓ N 条新消息" + 思考时间显示 "🧠 思考 X.Xs"
- 上下文进度条 "XX% context used" 带颜色警告

### 子Agent系统
- `agents/*.md` YAML frontmatter → Agent 类型注册
- `subagent` 工具：隔离上下文执行，支持同步/后台模式

### MCP 集成
- MCP stdio 客户端（JSON-RPC 2.0）
- `mcp` 工具：connect/disconnect/list/call，工具自动注册
- config.json 自动连接

### 工具集 (40+)
- 文件系统: read/write/edit/replace/glob/grep/move/copy/delete/mkdir
- 命令执行: bash/background（自愈+重试+超时终止）
- 浏览器: Playwright + HTTP 降级
- 网络: HTTP 请求/Web 搜索/WebSocket
- 系统: 进程/服务/注册表/GUI 自动化/监控
- 代码分析: AST/依赖图/调用链/错误诊断
- 记忆: 语义记忆（ChromaDB）
- 测试: 自动发现/运行/结果解析
- 定时: cron 任务调度
- 文件监控: file/directory/log/process 类型
...
- **README.md**: 全面重写为 v1.0 版本
- **examples/dashboard.html**: AgiCode 生成的 3D 全息仪表盘（Three.js）
