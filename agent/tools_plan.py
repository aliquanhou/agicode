                ...
                try:
                    ...
                    with _BT_LOCK:
                        if tid_inner in _BACKGROUND_TASKS:
                            ...
                            # 确保子进程被清理
                            proc = _BACKGROUND_TASKS[tid_inner].get("proc")
        ...
        if action == "create":
            ...
            if step_status == "in_progress":
                for dep in sl[step_index].get("depends_on", []):
                    dep_idx = int(dep) if not isinstance(dep, int) else dep
                    if dep_idx < len(sl) and sl[dep_idx].get("status") != "completed":
            ...
            if step_status == "completed":
                for i, s in enumerate(sl):
                    if s.get("status") == "pending" and all(
                        int(d) < len(sl) and sl[int(d)].get("status") == "completed" for d in s.get("depends_on", [])
                    ):
                        r += f"\n  下一步: step {i}「{s['step']}」"
                        break
            return r
