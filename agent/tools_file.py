...
def _handle_write(file_path: str = "", content: str = "") -> str:
    ...
    try:
        ...
        result = f"[写入成功] {os.path.getsize(abs_path)} 字节 → {file_path}"
        if diff_text:
            result += f"\n\n--- DIFF START ---\n{diff_text}\n--- DIFF END ---"
        return result
...
def _handle_edit(file_path: str = "", old_string: str = "", new_string: str = "") -> str:
    ...
    try:
        ...
        if old_normalized not in content_normalized:
            # 模糊匹配：尝试移除首尾空格
            old_stripped = old_normalized.strip()
            if old_stripped in content_normalized:
                old_normalized = old_stripped
        ...
        # 生成 Unified Diff 供 GUI 渲染
        rel_path = os.path.relpath(abs_path)
        old_lines = content_normalized.splitlines(keepends=True)
        new_lines = new_content_normalized.splitlines(keepends=True)
        diff_lines = list(difflib.unified_diff(
            old_lines, new_lines,
            fromfile=rel_path, tofile=rel_path,
            n=3,  # 3行上下文
