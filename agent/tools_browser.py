...
# ── 浏览器实例管理（线程局部存储，避免多Agent共享）──

_tls = threading.local()


def _get_browser_attr(attr: str, default=None):
    """线程安全获取浏览器属性。"""
    return getattr(_tls, attr, default)


def _set_browser_attr(attr: str, value):
    """线程安全设置浏览器属性。"""
    setattr(_tls, attr, value)


def _check_playwright():
    """检查 Playwright 浏览器是否可用（只检测一次，线程本地）。"""
    _pa = _get_browser_attr("playwright_available")
    if _pa is not None:
        return _pa
    try:
        # 检查 Chromium 二进制是否存在
        ...
        with sync_playwright() as p:
            ...
            browser.close()
        _set_browser_attr("playwright_available", True)
    except Exception:
        _set_browser_attr("playwright_available", False)
    return _get_browser_attr("playwright_available")


def _get_or_create_page():
    """获取或创建浏览器页面（线程安全，线程局部存储）。"""
    _browser = _get_browser_attr("browser")
    _browser_playwright = _get_browser_attr("browser_playwright")
    _browser_context = _get_browser_attr("browser_context")
    _browser_page = _get_browser_attr("browser_page")
    ...
    try:
        ...
        _browser_playwright_inst = sync_playwright().start()
        _browser = _browser_playwright_inst.chromium.launch(
            headless=False,
            args=[
        ...
        _set_browser_attr("browser_console_logs", [])
        _set_browser_attr("browser_network_errors", [])
        _set_browser_attr("browser_page_errors", [])
        _browser_page.on("console", lambda m: _get_browser_attr("browser_console_logs", []).append(f"[{m.type}] {m.text[:500]}"))
        _browser_page.on("pageerror", lambda e: _get_browser_attr("browser_page_errors", []).append(str(e)[:500]))
        _browser_page.on("requestfailed", lambda r: _get_browser_attr("browser_network_errors", []).append(
            f"{r.url[:200]} -> {str(r.failure) if r.failure else 'unknown'}"
        ))
        _set_browser_attr("browser", _browser)
        _set_browser_attr("browser_context", _browser_context)
        _set_browser_attr("browser_page", _browser_page)
        _set_browser_attr("browser_playwright", _browser_playwright_inst)
        return _browser_page
    ...
    try:
        if action in ("open", "navigate"):
            ...
            if len(encoded) > max_b64:
                return f"[BROWSER_SCREENSHOT len={len(encoded)} (截断, 前{max_b64})]\n{encoded[:max_b64]}"
            ...
            logs = "\n".join(_get_browser_attr("browser_console_logs", [])[-50:])
            return logs if logs else "(无 console 日志)"

        elif action == "network":
            errors = "\n".join(_get_browser_attr("browser_network_errors", [])[-50:])
            ...
            _browser_console_logs = _get_browser_attr("browser_console_logs", [])
            _browser_page_errors = _get_browser_attr("browser_page_errors", [])
            _browser_network_errors = _get_browser_attr("browser_network_errors", [])
